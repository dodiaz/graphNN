...
print('preprocessing finished and training is now commencing, time used:', t2-t1)
device = torch.device('cpu')

model = KernelNN(width,ker_width,depth,edge_features,node_features).cpu()  # .cuda()

optimizer = torch.optim.Adam(model.parameters(), lr=learning_rate, weight_decay=5e-4)
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=scheduler_step, gamma=scheduler_gamma)

myloss = LpLoss(size_average=False)
u_normalizer.cpu()     # .cuda()

print("now going into the actual model training")
