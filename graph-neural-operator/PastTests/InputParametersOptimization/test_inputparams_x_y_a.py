...
path_train_err = 'results/'+ path + 'train4.txt'                                      ################### changed this
path_test_err = 'results/'+ path + 'test4.txt'                                        ################### chagned this
...
for j in range(ntrain):
    for i in range(k):
        ...
        data_train.append(Data(x=torch.cat([grid, train_a[j, idx].reshape(-1, 1),
                                            ######train_a_gradx[j, idx].reshape(-1, 1),            ################ took out here
...
for j in range(ntest):
    ...
    data_test.append(Data(x=torch.cat([grid, test_a[j, idx].reshape(-1, 1),
                                       ######test_a_gradx[j, idx].reshape(-1, 1),    ############### took out here
...
plt.show()
plt.savefig('results/picture_nograd_nosmooth.png')     #################### changed this